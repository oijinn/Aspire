...
public class Technician_profile implements Initializable {
    ...
    private void password_validation() {
        ...
        if (user_password.isEmpty()){
            ...
            if (valid_password){
                ...
                Technician technician = new Technician(lbl_user_id.getText().toLowerCase(), user_password, txt_user_phone_number.getText());
    ...
    private void import_current_user_profile(){

        String user_id = Activity_log.get_current_user_id();

        Technician technician = new Technician();
        technician.find_user_with_id(user_id);

        lbl_user_name.setText(technician.get_name().toUpperCase());
        lbl_user_id.setText(technician.get_id().toUpperCase());
        lbl_user_ic.setText(technician.get_ic());

        lbl_user_email_address.setText(technician.get_email_address());
        txt_unmask_password.setText(technician.get_password());
        txt_mask_password.setText(technician.get_password());

        txt_user_phone_number.setText(technician.get_phone_number());
    }
        ...
        table.setRowFactory(tv -> {
            ...
            row.setOnMouseClicked(event -> {
                if (event.getClickCount() == 2 && (! row.isEmpty()) ) {
                    ...
                    change_scene(btn_update, "Technician_appointment_summary.fxml");
                }
            });
            return row ;
        });
